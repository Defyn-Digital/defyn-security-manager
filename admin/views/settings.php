...
?>
<?php
$notice = get_transient( 'dsm_settings_notice' );
if ( $notice ) {
	delete_transient( 'dsm_settings_notice' );
}
?>
<div class="wrap defyn-bem-wrap">
	...
	<h2 class="nav-tab-wrapper">
		<?php foreach ( $tabs as $key => $label ) :
			$url = admin_url( 'admin.php?page=' . DSM_SLUG . '&tab=' . $key );
	...
	<form method="post" action="options.php" class="defyn-bem-form">
		<?php settings_fields( 'dsm_settings_group' ); ?>
			...
			<table class="form-table" role="presentation">
				<tr>
					...
					<td>
						...
						<input type="text" id="login_slug" name="<?php echo esc_attr( DSM_OPTION ); ?>[login_slug]"
				...
				<tr>
					...
					<td>
						<label><input type="radio" name="<?php echo esc_attr( DSM_OPTION ); ?>[blocked_response]" value="404"
							<?php checked( $settings['blocked_response'], '404' ); ?> />
							<?php esc_html_e( 'Show a 404 (recommended)', 'defyn-security-manager' ); ?></label><br />
						<label><input type="radio" name="<?php echo esc_attr( DSM_OPTION ); ?>[blocked_response]" value="redirect"
							<?php checked( $settings['blocked_response'], 'redirect' ); ?> />
							<?php esc_html_e( 'Redirect to:', 'defyn-security-manager' ); ?></label>
						<input type="url" name="<?php echo esc_attr( DSM_OPTION ); ?>[blocked_redirect_url]"
						       value="<?php echo esc_attr( $settings['blocked_redirect_url'] ); ?>"
						       placeholder="<?php echo esc_attr( home_url( '/' ) ); ?>" class="regular-text" /><br />
						<label><input type="radio" name="<?php echo esc_attr( DSM_OPTION ); ?>[blocked_response]" value="fake"
				...
				<tr>
					...
					<td>
						<textarea id="ip_allowlist" name="<?php echo esc_attr( DSM_OPTION ); ?>[ip_allowlist]"
						...
						<p class="description">
							...
							<br /><?php esc_html_e( 'Behind a proxy? Define DSM_TRUST_PROXY in wp-config.php to honor X-Forwarded-For.', 'defyn-security-manager' ); ?>
						</p>
					</td>
				</tr>
				<tr>
					...
					<td>
						<label><input type="checkbox" name="<?php echo esc_attr( DSM_OPTION ); ?>[hide_xmlrpc]" value="1"
						...
						<label style="margin-top:10px;display:inline-block;"><input type="checkbox" name="<?php echo esc_attr( DSM_OPTION ); ?>[hide_rest_api]" value="1"
		...
		<?php elseif ( $active_tab === 'security' ) : ?>
			<table class="form-table" role="presentation">
				<tr>
					...
					<td>
						<label><input type="checkbox" name="<?php echo esc_attr( DSM_OPTION ); ?>[throttle_enabled]" value="1"
						...
						<p class="description" style="margin-top:8px;">
							...
							<input type="number" min="1" max="50" name="<?php echo esc_attr( DSM_OPTION ); ?>[throttle_max_attempts]"
							...
							<input type="number" min="1" max="1440" name="<?php echo esc_attr( DSM_OPTION ); ?>[throttle_window_min]"
							...
							<input type="number" min="1" max="10080" name="<?php echo esc_attr( DSM_OPTION ); ?>[throttle_lockout_min]"
							       value="<?php echo esc_attr( $settings['throttle_lockout_min'] ); ?>" class="small-text" />
							<?php esc_html_e( 'minutes.', 'defyn-security-manager' ); ?>
						</p>
						<?php
						$active_lockouts = DSM_Throttle::count_active();
						if ( $active_lockouts > 0 ) :
							...
							?></strong>
								<button type="submit" formaction="<?php echo esc_url( admin_url( 'admin.php' ) ); ?>"
								        formmethod="post" name="dsm_clear_lockouts" value="1"
								...
								<?php wp_nonce_field( 'dsm_clear_lockouts' ); ?>
							</p>
						<?php endif; ?>
					</td>
				</tr>
				<tr>
					...
					<td>
						<label><input type="checkbox" name="<?php echo esc_attr( DSM_OPTION ); ?>[time_window_enabled]" value="1"
						...
						<p style="margin-top:8px;">
							...
							<input type="time" name="<?php echo esc_attr( DSM_OPTION ); ?>[time_window_start]"
							       value="<?php echo esc_attr( $settings['time_window_start'] ); ?>" />
							<?php esc_html_e( 'to', 'defyn-security-manager' ); ?>
							<input type="time" name="<?php echo esc_attr( DSM_OPTION ); ?>[time_window_end]"
						...
						<p style="margin-top:8px;">
							...
							<?php foreach ( $days as $i => $label ) : ?>
								<label style="margin-right:8px;">
									<input type="checkbox" name="<?php echo esc_attr( DSM_OPTION ); ?>[time_window_days][]" value="<?php echo (int) $i; ?>"
						...
						<p>
							...
							<input type="text" id="emergency_bypass_code" name="<?php echo esc_attr( DSM_OPTION ); ?>[emergency_bypass_code]"
		...
		<?php elseif ( $active_tab === '2fa' ) : ?>
			<table class="form-table" role="presentation">
				<tr>
					...
					<td>
						<label><input type="checkbox" name="<?php echo esc_attr( DSM_OPTION ); ?>[two_factor_enabled]" value="1"
					...
					<td>
						<?php foreach ( $roles_all as $slug => $details ) : ?>
							<label style="display:block;margin-bottom:4px;">
								<input type="checkbox" name="<?php echo esc_attr( DSM_OPTION ); ?>[two_factor_required_roles][]"
		...
		<?php elseif ( $active_tab === 'alerts' ) : ?>
			<table class="form-table" role="presentation">
				<tr>
					...
					<td>
						<label><input type="checkbox" name="<?php echo esc_attr( DSM_OPTION ); ?>[alerts_enabled]" value="1"
				...
				<tr>
					...
					<td>
						<input type="email" id="alerts_email" name="<?php echo esc_attr( DSM_OPTION ); ?>[alerts_email]"
				...
				<tr>
					...
					<td>
						<label><input type="checkbox" name="<?php echo esc_attr( DSM_OPTION ); ?>[alerts_on_lockout]" value="1"
						...
						<label><input type="checkbox" name="<?php echo esc_attr( DSM_OPTION ); ?>[alerts_on_scan]" value="1"
						...
						<label><input type="checkbox" name="<?php echo esc_attr( DSM_OPTION ); ?>[alerts_on_new_ip_login] " value="1"
				...
				<tr>
					...
					<td>
						<input type="number" min="1" max="365" id="log_retention_days"
						       name="<?php echo esc_attr( DSM_OPTION ); ?>[log_retention_days]"
