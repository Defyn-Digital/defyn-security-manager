...
delete_option( 'defsec_settings' );
delete_option( 'defsec_db_version' );
delete_site_option( 'defsec_settings' );

$defsec_log_table  = $wpdb->prefix . 'defsec_log';
$defsec_lock_table = $wpdb->prefix . 'defsec_lockouts';
...
$wpdb->query( $wpdb->prepare( 'DROP TABLE IF EXISTS %i', $defsec_log_table ) );
$wpdb->query( $wpdb->prepare( 'DROP TABLE IF EXISTS %i', $defsec_lock_table ) );
$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->usermeta} WHERE meta_key LIKE %s", 'defsec\_%' ) );
